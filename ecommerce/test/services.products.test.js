...
describe("services - products", function() {
  /* Modify the constructor of ProductsService changing this.mongoDB = new MongoLib() by the MongoLibMock class */
  const ProductsService = proxyquire("../services/products", {
    // Instead of this  MongoLib = require("../lib/mongo") use this MongoLibMock
    "../lib/mongo": MongoLibMock
  });

  const productsService = new ProductsService();

  // describe.only can be used to test only one service
  describe("when getProducts method is called", async function() {
    it(" should call the getAll MongoLib method", async function() {
      await productsService.getProducts({});
      assert.strictEqual(getAllStub.called, true);  // Validate if getAllStub method in MongoLibMock is called
    });
